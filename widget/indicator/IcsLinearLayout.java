    ...
    protected void onDraw(Canvas canvas) {
        if (mDivider != null) {
            if (getOrientation() == VERTICAL) {
                drawDividersVertical2(canvas);
            } else {
                drawDividersHorizontal2(canvas);
            }
        }
        super.onDraw(canvas);
    }

    private void drawDividersVertical2(Canvas canvas) {
        final int count = getChildCount();
        for (int i = 0; i < count; i++) {
            ...
            if (child != null && child.getVisibility() != GONE) {
                if (hasDividerBeforeChildAt(i)) {
                    ...
                    drawHorizontalDivider2(canvas, top);
                }
            }
        }
        ...
        if (hasDividerBeforeChildAt(count)) {
            ...
            drawHorizontalDivider2(canvas, bottom);
        }
    }

    private void drawDividersHorizontal2(Canvas canvas) {
        final int count = getChildCount();
        for (int i = 0; i < count; i++) {
            final View child = getChildAt(i);
            ...
            if (child != null && child.getVisibility() != GONE) {
                if (hasDividerBeforeChildAt(i)) {
                    ...
                    drawVerticalDivider2(canvas, left);
                }
            }
        }
        ...
        if (hasDividerBeforeChildAt(count)) {
            ...
            drawVerticalDivider2(canvas, right);
        }
    }

    private void drawHorizontalDivider2(Canvas canvas, int top) {
        mDivider.setBounds(getPaddingLeft() + mDividerPadding, top,
                getWidth() - getPaddingRight() - mDividerPadding, top + mDividerHeight);
        mDivider.draw(canvas);
    }

    private void drawVerticalDivider2(Canvas canvas, int left) {
